...
// internal error", which is why plain fetch() from Vercel (IPv4 egress)
// fails. We connect to the IPv6 literal with the domain as SNI.
...
async function upstreamRequestOnce(
  ...
  const session = http2.connect(`https://[${BACKEND_IPV6}]`, {
    ALPNProtocols: ["h2"],
    servername: new URL(BACKEND).host,
  });
